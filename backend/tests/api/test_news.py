...
async def test_read_news_subscriptions_api(
    client: AsyncClient, test_news_subscription
) -> None:
...
async def test_read_news_subscriptions_by_user(
    client: AsyncClient, test_user, test_news_subscription
) -> None:
...
async def test_update_news_subscription_api(
    ...
    response = await client.put(
        f"{settings.API_V1_STR}/news/{test_news_subscription.id}", json=update_data
    )
    assert response.status_code == 200
    content = response.json()

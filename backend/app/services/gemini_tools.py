...
def build_system_instruction(
    ...
    dynamic_system_instruction = (
        ...
        f"1. For weather-only questions (current weather, forecasts, temperature, precipitation, even if mentioning 'today', 'tomorrow', or specific days/cities), delegate directly to WeatherAgent. For combined requests asking about both schedule/tasks and weather, or explicit daily briefings, follow rule 5.\n"
        f"2. For calendar, scheduling, email, inbox, or task/to-do list management questions, delegate to SecretaryAgent.\n"
        f"3. For questions about personal documents or knowledge base, delegate to KnowledgeAgent.\n"
        f"4. For general conversation that doesn't need any tools, respond directly without delegation.\n"
        f"5. Daily Briefing / Combined Requests: If the user asks for a daily briefing/agenda, or asks about BOTH their schedule/tasks and the weather (e.g., 'what does my day look like?', 'plan for today', 'what is scheduled today and what is the weather?', 'daily briefing', 'мій день'), delegate first to SecretaryAgent for schedule/tasks and then WeatherAgent for weather to provide a complete summary. Do NOT delegate to SecretaryAgent for queries that are purely about weather.\n"
